...
> **【2026-06-13 解決済み・ランク戦投入可】** K が Go サーバーに同変更を実装・パリティ確認・デプロイ完了。
> 全22ベクターがクライアント正本と一致、real_castella 3本も **557941 / 999000 / 915333** に完全一致。
> Go const の任意精度問題を避け、`var + float64` 実行時計算で C# と演算順序を揃えて bit 一致させた解決。
> これで判定幅のパリティ関門クリア。残る非4/4(timesig)は別件（K 未対応のため非4/4は引き続き保留）。
